...
import { readStoredUserInfo } from "@/app/hooks/_lib/storage/userInfo.storage";

export function LandingPageScreen() {
    const [hasKnownUser] = useState(() => Boolean(readStoredUserInfo()?.user_id));
    ...
    return (
        <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-slate-50 text-slate-900 selection:bg-blue-100 selection:text-blue-900">
            <div className="pointer-events-none absolute inset-0 z-0 opacity-40">
                <div className="absolute left-1/2 top-0 h-[1000px] w-[1000px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,rgba(219,234,254,0.5)_0%,rgba(255,255,255,0)_70%)] blur-[100px]" />
                <div className="absolute bottom-0 right-0 h-[800px] w-[800px] translate-x-1/4 translate-y-1/4 rounded-full bg-[radial-gradient(circle,rgba(224,242,254,0.3)_0%,rgba(255,255,255,0)_70%)] blur-[80px]" />
            </div>
            ...
            <main className="relative z-10 flex w-full max-w-5xl flex-col items-center gap-12 px-6 text-center">
                ...
                <motion.div
                    ...
                    className="pt-4"
                >
                    <Link href="/dynamic" className="group relative inline-flex">
                        <div className="relative flex h-14 items-center justify-center gap-3 overflow-hidden rounded-full bg-blue-600 px-10 text-base font-medium text-white shadow-lg shadow-blue-600/20 transition-transform duration-300 active:scale-95 group-hover:bg-blue-700">
                            <span>{hasKnownUser ? "Continue to assistant" : "Talk to our website"}</span>
                        ...
                        <div className="absolute left-4 right-4 top-2 -z-10 h-full rounded-full bg-blue-600/30 blur-xl transition-opacity duration-500 group-hover:opacity-50" />
                    </Link>
                    ...
                    >
                        ...
                        <p className="text-[10px] uppercase tracking-[0.25em] text-slate-400">Secure Session</p>
                        <span className="h-px w-12 bg-slate-200" />
                    </motion.div>
                </motion.div>
            </main>
