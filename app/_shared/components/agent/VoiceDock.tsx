...
interface VoiceDockProps {
    /** Current interaction mode from the engine. */
    mode: 'voice' | 'text';
    setInteractionMode: (mode: 'voice' | 'text') => void;
    /** Mute/unmute the local mic. true = muted. */
...
/**
 * VoiceDock — the control bar.
 *
 * The floating pill at the bottom: voice visualizer, agent-mute, mic toggle,
 * text input, and end-session. Owns only its own local UI state (typed text +
 * the two mute flags); all real work goes back through the engine handlers it's
 * given. Pairs with <Canvas>; the two are always mounted together (AgentInterface).
 */
    ...
    return (
        <div className={`relative z-30 flex flex-col justify-end flex-1 pointer-events-none ${isWindow ? 'mb-5' : 'mb-8'}`}>
            <div className={`pointer-events-auto flex w-full justify-center ${isWindow ? 'p-2' : 'p-4'}`}>
                <div className={`flex w-full items-center gap-1.5 rounded-[32px] bg-white/80 p-1.5 shadow-[0_20px_40px_rgba(0,0,0,0.08)] ring-1 ring-black/[0.04] backdrop-blur-2xl transition-all hover:scale-[1.01] hover:shadow-[0_25px_50px_rgba(0,0,0,0.12)] ${isWindow ? 'max-w-[400px] mx-auto gap-2 sm:gap-2 p-1.5 sm:p-1.5 sm:pl-2.5' : 'sm:w-auto sm:max-w-none sm:gap-3 sm:p-2 sm:pl-3'}`}>
