...
interface ChatWindowShellProps {
    ...
    /** Idle pause: agent ended the session but the widget stays open. Shows a resume prompt. */
    isPaused?: boolean;
    /** Called when the user taps mic / types in the paused state — starts a fresh session. */
    onResume?: () => void;
    /** Called when the LiveKit room disconnects on its own (idle) — distinct from a user close. */
    onRoomDisconnected?: () => void;
    /** Backend watchdog triggered a soft-pause without disconnecting the session. */
    onForcePause?: () => void;
                ...
                {!isOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Glossy top sheen */}
                            <span className="pointer-events-none absolute inset-0 rounded-full bg-linear-to-b from-white/25 via-transparent to-transparent" />
            ...
            <AnimatePresence onExitComplete={onExitComplete}>
                {isOpen && (
                    <motion.div
                        ...
                        className={isMobile
                            ? "fixed z-50 flex flex-col overflow-hidden inset-0 shadow-2xl ring-1 ring-sky-300/40"
                            : `fixed z-50 flex flex-col overflow-hidden bottom-6 right-6 h-[691px] max-h-[calc(100dvh-3rem)] rounded-[28px] backdrop-blur-2xl ring-1 ring-sky-300/50 shadow-[0_28px_80px_-12px_rgba(15,23,42,0.28)] ${isExpanded ? "w-[826px]" : "w-[461px]"}`
                        }
                        style={{
                            background: isMobile
                                ? "linear-gradient(to bottom, #60a5fa 0%, #93c5fd 15%, #bae6fd 35%, #e0f2fe 50%, #bae6fd 65%, #93c5fd 85%, #60a5fa 100%)"
                                : "linear-gradient(to bottom, rgba(96,165,250,0.95) 0%, rgba(147,197,253,0.92) 15%, rgba(186,230,253,0.88) 35%, rgba(224,242,254,0.86) 50%, rgba(186,230,253,0.88) 65%, rgba(147,197,253,0.92) 85%, rgba(96,165,250,0.95) 100%)",
                        }}
                    >
                        {/* Soft blue glow — the image's tint comes from a diffuse blue
                            wash pooling toward the lower half of the card. */}
                        <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
                            <div className="absolute -bottom-28 left-1/2 h-[440px] w-[440px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle,rgba(96,165,250,0.30)_0%,rgba(147,197,253,0)_70%)] blur-[70px]" />
                        </div>
                        ...
                        <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-center justify-between bg-linear-to-b from-sky-100/95 via-sky-100/50 to-transparent px-4 pb-6 pt-3">
                            ...
                            <div className="pointer-events-auto flex items-center gap-1">
                                ...
                                {onToggleExpand && (
                                    <button
                                        ...
                                        className={`group relative h-8 w-8 items-center justify-center overflow-hidden rounded-full backdrop-blur-md transition-all duration-200 hover:scale-105 active:scale-95 ${isMobile ? "hidden" : "flex"}`}
                                        style={{
                                            background: "linear-gradient(145deg, rgba(255,255,255,0.75) 0%, rgba(186,230,253,0.55) 50%, rgba(147,197,253,0.45) 100%)",
                                            boxShadow: "0 2px 8px rgba(59,130,246,0.18), inset 0 1px 0 rgba(255,255,255,0.9), inset 0 -1px 0 rgba(147,197,253,0.3)",
                                    ...
                                    >
                                        {/* Gloss sheen */}
                                        <span className="pointer-events-none absolute inset-x-0 top-0 h-1/2 rounded-t-full bg-linear-to-b from-white/60 to-transparent" />
                                        <span className="relative z-10 text-blue-600/90">
                                            {isExpanded ? (
                                ...
                                <button
                                    onClick={onClose}
                                    className="group relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-full backdrop-blur-md transition-all duration-200 hover:scale-105 active:scale-95"
                                    style={{
                                        background: "linear-gradient(145deg, rgba(255,255,255,0.75) 0%, rgba(254,202,202,0.55) 50%, rgba(252,165,165,0.45) 100%)",
                                        boxShadow: "0 2px 8px rgba(239,68,68,0.15), inset 0 1px 0 rgba(255,255,255,0.9), inset 0 -1px 0 rgba(252,165,165,0.3)",
                                        border: "1px solid rgba(255,255,255,0.7)",
                                    }}
                                ...
                                >
                                    {/* Gloss sheen */}
                                    <span className="pointer-events-none absolute inset-x-0 top-0 h-1/2 rounded-t-full bg-linear-to-b from-white/60 to-transparent" />
                                    <span className="relative z-10 text-red-400/90">
                            ...
                            {error ? (
                                ...
                                <div className="flex h-full flex-col items-center justify-center gap-8 px-6 text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        <p className="text-lg font-semibold text-slate-800">I&apos;m still here</p>
                                        <p className="max-w-60 text-sm leading-relaxed text-slate-500">
                                            Session paused. Start a new conversation with Vani.
                                        </p>
                                    </div>

                                    <button
                                        onClick={onResume}
                                        className="flex items-center gap-2.5 rounded-full bg-[#1a52a0] px-8 py-3.5 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(26,82,160,0.35)] transition-all hover:scale-105 hover:bg-[#154590] hover:shadow-[0_12px_32px_rgba(26,82,160,0.45)] active:scale-95"
                                    >
                                ...
                                <LiveKitRoom
                                    ...
                                    style={{ height: "100%" }}
                                    onDisconnected={onRoomDisconnected ?? onClose}
                                    onError={(err) => console.error("LiveKit Room Error:", err)}
                                >
                                    <AgentInterface variant="window" onDisconnect={onClose} onPause={onResume} onForcePause={onForcePause} isExpanded={isExpanded} />
                                </LiveKitRoom>
                            )}
                        </div>
                    </motion.div>
