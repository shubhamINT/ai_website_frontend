...
import { createBounds, decodePolyline } from '@/app/dynamic/_components/maps/map.utils';
import { EndIcon, StartIcon } from '@/app/dynamic/_components/maps/leaflet-icons';

const ChangeView = ({ bounds }: { bounds: ReturnType<typeof createBounds> }) => {
    const map = useMap();
...
export const MapDisplay = React.memo(({ polyline, origin, destination, travelMode, distance, duration }: MapDisplayProps) => {
    ...
    return (
        ...
        >
            <div className="absolute inset-0 z-10 pointer-events-none rounded-[30px] border-[6px] border-white/50 shadow-inner" />
                    ...
                    <div className="flex items-center gap-3 rounded-xl bg-white/90 px-3 py-1.5 text-zinc-900 shadow-lg ring-1 ring-black/5 backdrop-blur-md">
                        {distance && (
                            <div className="flex flex-col">
                                <span className="text-[8px] font-medium uppercase text-zinc-500">Distance</span>
                                <span className="text-xs font-bold">{distance}</span>
                            </div>
                        )}
                        {distance && duration && <div className="h-4 w-px bg-zinc-200" />}
                        {duration && (
                            <div className="flex flex-col">
                                <span className="text-[8px] font-medium uppercase text-zinc-500">Time</span>
                    ...
                    <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-3">
                            <div className="flex flex-col items-center">
                                ...
                                <div className="h-4 w-0.5 border-l border-dashed border-zinc-300" />
                                <div className="h-2.5 w-2.5 rounded-sm bg-red-500" />
                            </div>
                            <div className="flex-1 space-y-1">
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-medium uppercase tracking-tighter text-zinc-400">Your Location</span>
                                    <span className="max-w-[150px] truncate text-xs font-semibold text-zinc-900">{origin || 'Current Position'}</span>
                                </div>
                                <div className="h-px bg-zinc-100" />
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-medium uppercase tracking-tighter text-zinc-400">Destination</span>
                                    <span className="max-w-[200px] truncate text-xs font-semibold text-zinc-900">{destination || 'Calculating...'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
