...
import type { NearbyOfficesData } from '@/app/hooks/agentTypes';
...
export const NearbyOffices: React.FC<NearbyOfficesProps> = ({ data }) => {
    ...
    return (
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex w-full flex-col items-center gap-6 px-4 py-8 md:px-8">
            ...
            <div className={`grid w-full gap-6 ${offices.length === 1 ? 'max-w-xl grid-cols-1' : offices.length === 2 ? 'max-w-4xl grid-cols-1 md:grid-cols-2' : 'max-w-6xl grid-cols-1 md:grid-cols-2 lg:grid-cols-3'}`}>
                {offices.map((office) => (
                    <motion.div
                        key={office.id}
                        variants={itemVariants}
                        whileHover={{ y: -5, transition: { duration: 0.2 } }}
                        className="group relative flex flex-col overflow-hidden rounded-[2rem] bg-white/95 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.06)] ring-1 ring-white/60 backdrop-blur-2xl transition-all hover:shadow-[0_40px_80px_rgba(0,0,0,0.12)] hover:ring-blue-100"
                    >
